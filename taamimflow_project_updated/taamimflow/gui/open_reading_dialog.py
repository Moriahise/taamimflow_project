...
class OpenReadingDialog(QDialog):
    ...
    def __init__(self, parent=None, xml_path: str | None = None) -> None:
        ...
        # Load sedrot
        try:
            # Determine where to load sedrot.xml from.  If an explicit
            # xml_path is provided use it; otherwise search standard
            # locations via find_data_file.  Path objects convert to
            # strings as needed by load_sedrot.
            if xml_path:
                path = Path(xml_path)
            else:
                path = find_data_file("sedrot.xml")
